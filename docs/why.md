...
`go-ruby-railties/railties` reimplements Ruby's `railties` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-railties/railties` directly, with no Ruby runtime;
...
Because `railties` is pure Go with cgo disabled, it also compiles to **WebAssembly**
